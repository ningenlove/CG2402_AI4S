...
parser.add_argument('-r', '--result', default='Result/Burgers/simulation_3', type=str, metavar='PATH', help='path to sabe results')
parser.add_argument('--num_epochs', default=19000, type=int, metavar='N', help='number of total epochs to run')
...
parser.add_argument('--milestones', type=int, nargs='+', default=[8000, 12000, 15000, 18000], help='decrease learning rate at these epochs')
...
plt.scatter(traindata_shock.SmpPts_Shock[:,0], traindata_shock.SmpPts_Shock[:,1], c = 'pink', label = 'characteristic line')
...
def train_epoch(epoch, model, optimizer, device):
    ...
    for i, (data_intrr, data_bndry, data_init, data_shock) in enumerate(zip(dataloader_intrr, cycle(dataloader_bndry), cycle(dataloader_init), cycle(dataloader_shock))):
        ...
        smppts_shockl = torch.cat([smppts_shock, Exact_Solution.level_set(smppts_shock[:,0] - 0.0001, smppts_shock[:,1]).reshape(-1,1)], dim=1)
        smppts_shockr = torch.cat([smppts_shock, Exact_Solution.level_set(smppts_shock[:,0] + 0.0001, smppts_shock[:,1]).reshape(-1,1)], dim=1)
        ...
        u_NN_shockl = model(smppts_shockl)
        u_NN_shockr = model(smppts_shockr)
        ...
        loss_intrr = torch.mean(torch.pow(gradu_NN_intrr[:,1] + 11 * torch.squeeze(u_NN_intrr) * gradu_NN_intrr[:,0], 2))
        loss_bndry = torch.mean(torch.pow(u_NN_bndryl - 1, 2)) + torch.mean(torch.pow(u_NN_bndryr, 2))
        loss_init = torch.mean(torch.pow(torch.squeeze(u_NN_init) - f_smppts, 2))

        # c/2 * (u_l - u_r) - (f(u_l) -f(u_r))
        loss_shock = torch.mean(torch.pow(0.5 * (u_NN_shockl + u_NN_shockr) - 0.5, 2))


        loss_minibatch = 10 * loss_intrr + args.beta * (loss_init + loss_shock + loss_bndry)
        ...
        loss_epoch += loss_intrr_epoch + loss_bndry_epoch + args.beta * (loss_init_epoch + loss_shock_epoch) 
...
with torch.no_grad():
    x = torch.linspace(0, 1, steps=args.num_test_x) * 7 - 1
    t = torch.ones(x.size())

    test_smppts = torch.cat([x.reshape(-1, 1), t.reshape(-1, 1)], dim=1)
    test_smppts = torch.cat([test_smppts, Exact_Solution.level_set(test_smppts[:,0], test_smppts[:,1]).reshape(-1, 1)], dim=1)

    init_smppts = torch.cat([x.reshape(-1, 1), 0*t.reshape(-1, 1)], dim=1)
...
fig=plt.figure()
u_Exact = Exact_Solution.u_Exact_Solution(test_smppts[:,0].cpu(),test_smppts[:,1].cpu().cpu()).detach().numpy().reshape(args.num_test_x, 1)
...
t = torch.linspace(0, 1, steps=args.num_test_t) * 5
