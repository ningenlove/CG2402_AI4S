...
def level_set(x,t):
    level = torch.zeros(x.size())
    mask = t <= 4
    temp1 = torch.ones(x[mask].size(0))
    level1 = torch.heaviside(x[mask] - t[mask]/4 - 1, torch.squeeze(temp1))
    level[mask] = level1

    mask = torch.logical_not(mask)
    temp2 = torch.ones(x[mask].size(0))
    level2 = torch.heaviside(x[mask] - torch.sqrt(t[mask]), torch.squeeze(temp2))
    level[mask] = level2
    return level
